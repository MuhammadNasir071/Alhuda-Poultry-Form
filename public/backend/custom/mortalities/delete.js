api['deleteMortality'] = ajax_path + '/admin/mortalities/:id';

$(function(){
    $('body').on('click', '#delete-mortality', function() {
        deleteMortality($(this));
    });
    function deleteMortality(data) {
        if ($(data).attr("data-id")) {
            let remove_id = data.data("id");
            let url = api['deleteMortality'];
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.responseCode === 200) {
                                toastr.success('Mortality deleted successfully');
