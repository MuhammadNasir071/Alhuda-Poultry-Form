api['deleteFeeds'] = ajax_path + '/admin/feeds/:id';

$(function(){
    $('body').on('click', '.delete_feed', function() {
        deleteShed($(this));
    });
    function deleteShed(data) {
        if ($(data).attr("data-id")) {
            let remove_id = data.data("id");
            let url = api['deleteFeeds'];
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.responseCode === 200) {
                                toastr.success('Feed deleted successfully');
